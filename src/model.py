...
def get_bert(bert_name):
    ...
    #ADDED FOR DISTILBERT
        ...
        model_config.output_hidden_states = True
        bert = DistilBertModel.from_pretrained('distilbert-base-uncased', config=model_config)
    
    #ADDED MobileBERT
    elif 'mobilebert' in bert_name:
        print('load mobilebert-uncased')
        model_config = MobileBertConfig.from_pretrained('google/mobilebert-uncased')
        model_config.output_hidden_states = True
        bert = MobileBertModel.from_pretrained('google/mobilebert-uncased', config=model_config)

    #ADDED ELECTRA
    elif 'electra' in bert_name:
        print('load electra-base-discriminator')
        model_config = ElectraConfig.from_pretrained('google/electra-base-discriminator')
        model_config.output_hidden_states = True
        bert = ElectraModel.from_pretrained('google/electra-base-discriminator', config=model_config)
    
    #ADDED TINYBERT
    elif 'tinybert' in bert_name:
        print('load TinyBERT')
        model_config = BertConfig.from_pretrained('huawei-noah/TinyBERT_General_4L_312D')
        model_config.output_hidden_states = True
        bert = BertModel.from_pretrained('huawei-noah/TinyBERT_General_4L_312D', config=model_config)
    #ADDED ALBERT
    elif 'albert' in bert_name:
        print('load ALBERT')
        model_config = AlbertConfig.from_pretrained('albert-base-v2')
        model_config.output_hidden_states = True
        bert = AlbertModel.from_pretrained('albert-base-v2', config=model_config)
    #ADDED MINILM
    elif 'minilm' in bert_name:
        print('load MiniLM')
...
class LightXML(nn.Module):
    def __init__(self, n_labels, group_y=None, bert='bert-base', feature_layers=5, dropout=0.5, update_count=1,
        ...
        if 'T5' in self.bert_name:
            hidden_size = self.bert.config.d_model
        elif 'minilm' in self.bert_name:
            hidden_size = self.bert.config.hidden_size
        else:
            hidden_size = self.feature_layers * self.bert.config.hidden_size

        if group_y is not None:
            self.group_y_labels = group_y.shape[0]
            self.l0 = nn.Linear(hidden_size, self.group_y_labels)
            self.l1 = nn.Linear(hidden_size, hidden_dim)
            self.embed = nn.Embedding(n_labels, hidden_dim)
            nn.init.xavier_uniform_(self.embed.weight)
        else:
            self.l0 = nn.Linear(hidden_size, n_labels)  
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        """
        # ADDED
        # Handle the T5 model output differently
        if 'T5' in self.bert_name:
            outs = self.bert(
                input_ids=input_ids,
                attention_mask=attention_mask
            ...
            out = torch.mean(outs, dim=1)  # Simplified example; adjust based on your needs
        elif  'minilm' in self.bert_name:
            # MiniLM might return a tuple with only the last hidden state
    ...
    def swa_step(self):
        if 'models_num' not in self.swa_state:
            return
        self.swa_state['models_num'] += 1
        betaa = 1.0 / self.swa_state['models_num']
        with torch.no_grad():
            for n, p in self.named_parameters():
                self.swa_state[n].mul_(1.0 - betaa).add_(betaa, p.data.cpu())
    ...
    def get_fast_tokenizer(self):
        ...
        #ADDED MobileBERT
        elif 'mobilebert' in self.bert_name:
        ...
        #ADDED ELECTRA
        elif 'electra' in self.bert_name:
            print('load electra-base-discriminator tokenizer')
            tokenizer = ElectraTokenizer.from_pretrained('google/electra-base-discriminator')
        
        #ADDED tinybert
        elif 'tinybert' in self.bert_name:
            tokenizer = BertTokenizer.from_pretrained('huawei-noah/TinyBERT_General_4L_312D', do_lower_case=True)
        
        #ADDED ALBERT
        elif 'albert' in self.bert_name:
    ...
    def get_tokenizer(self):
        ...
        #ADDED tinybert
            ...
            print('load TinyBERT tokenizer')
            tokenizer = BertTokenizer.from_pretrained('huawei-noah/TinyBERT_General_4L_312D', do_lower_case=True)
        #ADDED ALBERT
        elif 'albert' in self.bert_name:
            print('load ALBERT tokenizer')
            tokenizer = AlbertTokenizer.from_pretrained('albert-base-v2', do_lower_case=True)

        #ADDED minilm
        elif 'minilm' in self.bert_name:
            print('load MiniLM tokenizer')
            tokenizer = AutoTokenizer.from_pretrained('microsoft/MiniLM-L12-H384-uncased')
